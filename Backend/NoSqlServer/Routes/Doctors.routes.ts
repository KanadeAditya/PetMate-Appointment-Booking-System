...
DoctorRouter.post("/register", async (req: Request, res: Response) => {
  try {
    ...
    if (!name || !email || !password ||!UPRN) {
      ...
      const doctor = new DoctorModel({ name, email, password: hashedPassword, UPRN });
...
DoctorRouter.post("/login", async (req: Request, res: Response) => {
  try {
    ...
    const acessToken = jwt.sign(
      { userID: doc._id, role: doc.role, email: doc.email },
    ...
    const refToken = jwt.sign(
      { userID: doc._id, role: doc.role, email: doc.email, status: doc.status },
    ...
    res
      .status(200)
      .send({ name: doc.name, email: doc.email, acessToken, refToken });
  } catch (err) {
...
//updating the remaining field of doctor on this route
DoctorRouter.patch('/info', AuthMiddleware, async (req: Request, res: Response) => {
  try {
